...
`${gateway.gatewayId.activations} < 10` or whatever syntax we decide for. `activations` serves as a local varibale for keep track onm how many times the gateway was invoked (there are many ways to express that, e.g. we could call the variable also)
